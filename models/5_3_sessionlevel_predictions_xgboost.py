...
df_train['predictions_xgboost'] = np.expm1(oof_reg_preds)
df_train['totals.transactionRevenue'] = y_reg 
df_test['predictions_xgboost'] = sub_reg_preds
df_test["fullVisitorId"] = df_test_fullvisitorid_str
df_train[pass_columns].to_csv("../input/df_train_session_xgboost.csv", index = False)
df_test[pass_columns].to_csv("../input/df_test_session_xgboost.csv", index = False)
